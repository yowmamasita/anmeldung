...
async function processAppointmentURL(browser, elem) {
	let appointmentURL = elem.querySelector('a').href;
	if (/(Schöneweide|Köpenick|Blaschkoallee|Neukölln|Sonnenallee|Zwickauer|Rudow)/.test(elem.innerText)) {
...
async function getAppointmentSlots(browser, dateUrl) {
	...
	const appointmentSlots = await page.evaluate(() => {
		...
		const appointmentSchedule = (new Date(parseInt(lastSegment, 10) * 1000))
